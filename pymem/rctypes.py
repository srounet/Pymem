# This source code has taken from PythonForWindows project, all the credit goes to the authors.
# It was published under BSD 3-Clause "New" or "Revised" License licence.
# https://github.com/hakril/PythonForWindows/blob/master/windows/remotectypes.py
...
class RemoteValue(object):
    @classmethod
    def from_buffer_with_target(cls, buffer, offset=0, handle=None):
...
class RemoteCCharP(RemotePtr, ctypes.c_char_p):
    ...
    def value(self):
        ...
        for i in itertools.count():
            x = pymem.memory.read_bytes(self.handle, base + (i * 0x100), 0x100)
            if b"\x00" in x:
...
class RemoteStructurePointer(RemotePtr, ctypes.c_void_p):
    @classmethod
    def from_buffer_with_target_and_ptr_type(cls, buffer, offset=0, handle=None, ptr_type=None):
        x = cls.from_buffer(buffer)
        x.handle = handle
    ...
    def contents(self):
        ...
        return remote_pointed_type(self.handle, self.raw_value)
...
def create_remote_array(subtype, len):
    ...
    class RemoteArray(_ctypes.Array):
        ...
        def __getitem__(self, slice):
            ...
            item_addr = self.base_address + (ctypes.sizeof(subtype) * slice)

            # TODO: do better ?
            class TST(ctypes.Structure):
                _fields_ = [("TST", subtype)]
            return RemoteStructure.from_structure(TST)(self.handle, item_addr).TST
...
class RemoteStructureUnion(object):
    ...
    _reserved_name = ["handle", "_fields_", "_fields_dict_", "base_address", "_get_field_by_name",
    ...
    def _handle_field_getattr(self, ftype, fosset, fsize):
        s = pymem.memory.read_bytes(self.handle, self.base_address + fosset, fsize)
        if ftype in self._field_type_to_remote_type:
            return self._field_type_to_remote_type[ftype].from_buffer_with_target(bytearray(s), handle=self.handle).value
        if issubclass(ftype, _ctypes._Pointer):  # Pointer
            return RemoteStructurePointer.from_buffer_with_target_and_ptr_type(bytearray(s), handle=self.handle, ptr_type=ftype)
        if issubclass(ftype, RemotePtr64):  # Pointer to remote64 bits process
            return RemoteStructurePointer64.from_buffer_with_target_and_ptr_type(bytearray(s), handle=self.handle, ptr_type=ftype)
        if issubclass(ftype, RemotePtr32):  # Pointer to remote32 bits process
            return RemoteStructurePointer32.from_buffer_with_target_and_ptr_type(bytearray(s), handle=self.handle, ptr_type=ftype)
        if issubclass(ftype, RemoteStructureUnion):  # Structure|Union already transfomed in remote
            return ftype(self.handle, self.base_address + fosset)
        if issubclass(ftype, ctypes.Structure):  # Structure that must be transfomed
            return RemoteStructure.from_structure(ftype)(self.handle, self.base_address + fosset)
        if issubclass(ftype, ctypes.Union):  # Union that must be transfomed
            return RemoteUnion.from_structure(ftype)(self.handle, self.base_address + fosset)
        if issubclass(ftype, _ctypes.Array):  # Arrays
            ...
            return create_remote_array(ftype._type_, ftype._length_)(self.handle, self.base_address + fosset)
        # Normal types
...
def MakePtr64(type):
    class PointerToStruct64(Remote_c_void_p64):
        ...
        @property
        def contents(self):
            return RemoteStructurePointer64.from_buffer_with_target_and_ptr_type(bytearray(self), handle=self.handle, ptr_type=self).contents
...
def MakePtr32(type):
    class PointerToStruct32(Remote_c_void_p32):
        ...
        @property
        def contents(self):
            return RemoteStructurePointer32.from_buffer_with_target_and_ptr_type(bytearray(self), handle=self.handle, ptr_type=self).contents
